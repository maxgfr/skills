...
for (const host of ['codex', 'claude']) {
  test(`doctor reports an actionable healthy ${host} installation contract`, () => {
    ...
    assert.equal(out.checks.find((check) => check.id === 'manual-policy').ok, true)
    assert.equal(out.checks.find((check) => check.id === 'explicit-invocation').ok, true)
    assert.equal(out.checks.find((check) => check.id === 'hooks').detail.endsWith('0 registered hooks'), true)
